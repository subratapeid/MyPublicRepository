...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // logData($logData);
// Check if the data to be updated is different from the existing data
$selectQuery = "SELECT bc_temp_email, bc_temp_mobile FROM bc_details WHERE bc_id = :userId";
...
if ($existingData) {
    // Data exists, compare with new data
    if ($existingData['bc_temp_email'] !== $email || $existingData['bc_temp_mobile'] !== $userMobile) {
    // Insert data into the database
   // UPDATE query
$updateQuery = "UPDATE bc_details SET bc_temp_email = :email, bc_temp_mobile = :userMobile WHERE bc_id = :userId";
$updateStmt = $pdo->prepare($updateQuery);
$updateStmt->bindParam(':email', $email, PDO::PARAM_STR);
$updateStmt->bindParam(':userMobile', $userMobile, PDO::PARAM_STR);
...
try {
    ...
    if ($updateSuccess) {
        ...
        $errorMessage = 'An error occurred while updating bc_details.';
        echo json_encode(['error' => $errorMessage]);
        // error_log('Error: ' . $errorMessage, 3, $logFilePath);
    }
