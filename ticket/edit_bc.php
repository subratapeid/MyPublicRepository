...
if (isset($_POST['userId']) && !empty($_POST['userId'])) {
  ...
  echo '<script>window.location.href = "all-bc-list.php";</script>';
  exit();  // Stop further execution
}
...
$userId = isset($_POST['userId']) ? $_POST['userId'] : null;
    ...
    if ($userId) {
        ...
        $stmt = $pdo->prepare('SELECT * FROM bc_details WHERE id = :userId');
        $stmt->execute(['userId' => $userId]);
        $user = $stmt->fetch();

        if (!$userId) {
            // Ticket not found, display alert and redirect to homepage
            echo '<script>alert("BC not found."); window.location.href = "all-bc-list.php";</script>';
            exit;  // Stop further execution
        }
      }
?>
...
<div class="card-header py-0 px-2">
      <h2 class="text-center">Edit BC Details</h2>
    </div>
<div class="card-body">
...
<script>
    // Function to make a POST request to fetch bc data
    function fetchUserData() {
        var apiUrl = 'codes/get_bc_details.php'; // Relative path to your PHP API endpoint
    ...
    function populateUserData(user) {
        document.getElementById('firstName').value = user.bc_first_name;
        document.getElementById('lastName').value = user.bc_last_name;
        document.getElementById('email').value = user.bc_temp_email;
        document.getElementById('pinInput').value = user.bc_temp_mobile;
